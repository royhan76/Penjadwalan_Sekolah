...
<head>
    ...
    <title>Jam Pelajaran — Penjadwalan Sekolah</title>
...
<main class="main-content">
    <div class="page-header animate-fadeInDown">
        <div><h1 class="page-title">🕐 Jam Pelajaran</h1><p class="page-subtitle">Atur slot waktu jam pelajaran</p></div>
    ...
    <div class="table-card glass-card animate-fadeInUp">
        <div class="table-toolbar">
            <input type="search" id="tableSearch" class="form-control search-input" placeholder="🔍 Cari jam...">
            <span class="table-count"><?= count($data) ?> slot waktu</span>
        </div>
        <div class="table-wrapper">
            <table class="data-table" id="dataTable">
                <thead><tr><th>No</th><th>ID</th><th>Label</th><th>Mulai</th><th>Selesai</th><th>Aksi</th></tr></thead>
                <tbody>
                    <?php if (empty($data)): ?>
                    <tr><td colspan="6" class="empty-row">Belum ada jam. Klik "Tambah Jam".</td></tr>
                    <?php else: ?>
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="action-btns">
                                <button class="btn btn-sm btn-edit" onclick="openModal('edit', <?= htmlspecialchars(json_encode(['row' => $row['_row'], 'id' => $row['ID'], 'label' => $row['Label'], 'mulai' => $row['Mulai'], 'selesai' => $row['Selesai']])) ?>)">✏️</button>
                                <button class="btn btn-sm btn-delete" onclick="confirmDelete(<?= $row['_row'] ?>, '<?= htmlspecialchars($row['Label']) ?>')">🗑️</button>
                            </div>
                        </td>
                    </tr>
                    <?php endforeach; ?>
...
<div class="modal-overlay" id="modalOverlay" onclick="closeModal(event)">
    <div class="modal glass-card">
        <div class="modal-header">
            <h3 class="modal-title" id="modalTitle">Tambah Jam Pelajaran</h3>
        ...
        <form id="mainForm" onsubmit="submitForm(event)">
            ...
            <div class="form-group">
                ...
                <input type="text" id="fieldLabel" name="label" class="form-control" placeholder="Contoh: Jam ke-1" required>
            </div>
            <div class="form-row">
                <div class="form-group">
                    <label class="form-label">Mulai <span class="required">*</span></label>
                    <input type="time" id="fieldMulai" name="mulai" class="form-control" required>
                </div>
                <div class="form-group">
                    <label class="form-label">Selesai <span class="required">*</span></label>
                    <input type="time" id="fieldSelesai" name="selesai" class="form-control" required>
                </div>
            </div>
...
<div class="modal-overlay" id="deleteOverlay" onclick="closeDelete(event)">
    <div class="modal glass-card" style="max-width:420px">
        ...
        <div class="modal-body"><p>Yakin hapus jam <strong id="deleteName"></strong>?</p></div>
        <div class="modal-footer">
...
<script>
const ENDPOINT = '/jam.php';
let deleteRow = null;
function openModal(mode, data = null) {
    document.getElementById('fieldAction').value = mode === 'edit' ? 'update' : 'store';
    document.getElementById('modalTitle').textContent = mode === 'edit' ? 'Edit Jam' : 'Tambah Jam';
    document.getElementById('submitText').textContent = mode === 'edit' ? 'Perbarui' : 'Simpan';
    if (data) { document.getElementById('fieldRow').value = data.row; document.getElementById('fieldId').value = data.id; document.getElementById('fieldLabel').value = data.label; document.getElementById('fieldMulai').value = data.mulai; document.getElementById('fieldSelesai').value = data.selesai; }
