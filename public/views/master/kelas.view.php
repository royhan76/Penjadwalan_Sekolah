...
<head>
    ...
    <title>Kelas — Penjadwalan Sekolah</title>
...
<main class="main-content">
    <div class="page-header animate-fadeInDown">
        <div><h1 class="page-title">🏫 Data Kelas</h1><p class="page-subtitle">Kelola daftar kelas</p></div>
    ...
    <div class="table-card glass-card animate-fadeInUp">
        <div class="table-toolbar">
            <input type="search" id="tableSearch" class="form-control search-input" placeholder="🔍 Cari kelas...">
            <span class="table-count"><?= count($data) ?> kelas terdaftar</span>
        </div>
        <div class="table-wrapper">
            <table class="data-table" id="dataTable">
                <thead><tr><th>No</th><th>ID</th><th>Nama Kelas</th><th>Aksi</th></tr></thead>
                <tbody>
                    <?php if (empty($data)): ?>
                    <tr><td colspan="4" class="empty-row">Belum ada kelas. Klik "Tambah Kelas".</td></tr>
                    <?php else: ?>
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="action-btns">
                                <button class="btn btn-sm btn-edit" onclick="openModal('edit', <?= htmlspecialchars(json_encode(['row' => $row['_row'], 'id' => $row['ID'], 'nama' => $row['Nama']])) ?>)">✏️</button>
    ...
    <div class="modal glass-card" id="mainModal">
        ...
        <form id="mainForm" onsubmit="submitForm(event)">
            ...
            <div class="form-group">
                ...
                <input type="text" id="fieldNama" name="nama" class="form-control" placeholder="Contoh: X IPA 1" required>
            </div>
...
<div class="modal-overlay" id="deleteOverlay" onclick="closeDelete(event)">
    <div class="modal glass-card" style="max-width:420px">
        ...
        <div class="modal-body"><p>Yakin hapus kelas <strong id="deleteName"></strong>?</p></div>
        <div class="modal-footer">
...
<script>
const ENDPOINT = '/kelas.php';
let deleteRow = null;
function openModal(mode, data = null) {
    document.getElementById('fieldAction').value = mode === 'edit' ? 'update' : 'store';
    document.getElementById('modalTitle').textContent = mode === 'edit' ? 'Edit Kelas' : 'Tambah Kelas';
    document.getElementById('submitText').textContent = mode === 'edit' ? 'Perbarui' : 'Simpan';
    if (data) { document.getElementById('fieldRow').value = data.row; document.getElementById('fieldId').value = data.id; document.getElementById('fieldNama').value = data.nama; }
